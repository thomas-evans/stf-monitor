...
export class FullSeriesService {
  public getFullSeries(series$: Subject<string>) {
    const subject: ReplaySubject<seriesData> = new ReplaySubject();
  ...
  private getFromOFR(series: string): Observable<seriesData> {
    return this.http
      .get<FullSeries>(
        `https://data.financialresearch.gov/v1/series/full?mnemonic=${series}`
      )
      .pipe(map((value) => Object.values(value)[0]));
  }
